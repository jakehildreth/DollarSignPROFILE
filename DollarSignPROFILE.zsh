...
__dollarsign_autoupdate() {
    ...
    local self_update_url='https://raw.githubusercontent.com/jakehildreth/DollarSignPROFILE/refs/heads/main/DollarSignPROFILE.zsh'
    local self_path="${ZDOTDIR:-$HOME}/.zshrc"
    ...
    local options=('Yes, always' 'Yes, just this time' 'No, not this time' 'No, never' 'More details')
    local reply
    PS3='Your choice: '
    select _choice in "${options[@]}"; do
        case "$REPLY" in
            1)
                printf '# DollarSignPROFILE:AutoUpdate=always\n%s' "$remote_content" > "$self_path"
                exec zsh -l
